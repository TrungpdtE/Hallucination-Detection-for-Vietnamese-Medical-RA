...
- truy xuất tài liệu y tế liên quan từ tri thức có sẵn (KB).
- có đưa ngữ cảnh truy xuất được đi vào mô hình sinh ra câu trả lời.
- kiểm tra câu trả lời đã tạo  bằng bộ phân loại halucination.
- đánh giá định lượng bằng retrieval metric, generation metric, hallucination detection metric.

Dataset gốc thuộc miền biomedical QA tiếng Anh (được lấy từ nguồn 1 nghiên cứu khác), sau đó được dịch và chuẩn hóa sang tiếng Việt (xài deepseek call api xử lí số lượng lớn) theo định dạng json dùng trong project.

## Tóm Tắt Lại

Trong các hệ thống hỏi đáp y tế ta có thể thấy lỗi hallucination đặc biệt nghiêm trọng vì "mô hình ngôn ngữ" có thể sinh ra thông tin nghe có vẻ hợp lý nhưng không có/được tài liệu hỗ trợ -> hiện tượng ảo giác. Project này tách hệ thống thành hai phần chính (module):

1. **RAG pipeline/module**: gồm retriever và LLM generator. Retriever tìm tài liệu liên quan còn LLM generator dùng tài liệu đó để sinh câu trả lời.
2. **Hallucination Detection module**: dùng PhoBERT fine-tuned để kiểm tra câu trả lời sau khi sinh.

Lưu ý là phoBERT trong project này **KHÔNG sinh câu trả lời**. PhoBERT đóng vai trò như là 1 verifier -> nó nhận vào:
...
Sau khi sửa lỗi formatter làm mất phần answer do truncation (đầu vào 256/512 cắt mất đi phần thông tin quan trongk) và train lại PhoBERT:
...
Ma trận nhầm lẫn trên toàn bộ benchmark:
...
Kết quả này cải thiện mạnh so với checkpoint cũ đã làm -> vốn gần như dự đoán toàn bộ sample là `supported` và có hallucinated recall bằng 0.
...
Tuy nhiên, việc có thêm context (bối cảnh thông tin) không bảo đảm câu trả lời luôn đúng được, thực ra LLM vẫn có thể:

- thêm thông tin không có trong tài liệu.
- hiểu sai quan hệ nguyên nhân <-> kết quả.
- tự ý thay đổi số liệu -  liều lượng - thời gian.
- phủ nhận sai một kết luận y khoa nào đó
- tự ý trộn thông tin từ nhiều tài liệu khác nhau.

Vì vậy project em đã bổ sung 1 tầng kiểm chứng:

```text
Question + Answer + Context -> PhoBERT verifier -> supported or hallucinated
```
Trước hết nên tìm hiểu thêm về RAG là gì? 

## RAG Là Gì?

Source url: [https://viblo.asia/p/tim-hieu-ve-retrieval-augmented-generation-rag-Ny0VGRd7LP]

RAG: viết tắt của Retrieval Augmented Generation -> nó là 1 phương pháp kết hợp truy xuất tài liệu với mô hình sinh ngôn ngữ. Thay vì yêu cầu LLM trả lời hoàn toàn từ kiến thức tham số đã học thì hệ thống trước tiên tìm các tài liệu liên quan trong knowledge base (tri thức có sẵn), sau đó thì đưa các TÀI LIỆU này vào prompt để LLM sinh câu trả lời có căn cứ hơn.

### Xét Về Trực Giác

Nếu như không có RAG sẽ như nào? 
...
Trong code hiện tại thì thực ra có hai phần retrieval tương ứng

- `src/rag/query_rag.py` -> pipeline RAG chạy trực tiếp bằng dense retrieval từ Chroma vector store.
- `src/rag/hybrid_retriever.py` -> module hybrid BM25 kết hợp dense dùng để kiểm thử/truy xuất độc lập. Có thể tích hợp vào `query_rag.py` trong bước cải tiến tiếp theo.

Vì vậy, khi nói **RAG pipeline/module** thì cần hiểu là toàn bộ chuỗi `retriever + prompt + LLM generator`. 
Retriever có thể là dense retriever hiện đang chạy trong `query_rag.py`, hoặc hybrid retriever nếu được tích hợp.
...
RAG là phần **retriever kết hợp generator**. PhoBERT verifier thì là module riêng nằm sau RAG.
...
Module verifier giúp trả lời câu hỏi này: **câu trả lời này có được ngữ cảnh truy xuất hỗ trợ hay không?**
...
Transformer encoder có giới hạn độ dài đầu vào (256/512). Với PhoBERT thì script tự điều chỉnh `max_length` theo khả năng của model
-> trong log thực nghiệm thì giá trị sử dụng là:
...
Context thường rất dài nên answer ở cuối sequence có thể bị truncate (mất data). Khi answer bị mất thì model chỉ nhìn thấy question và phần đầu context. Hai sample `supported` và `hallucinated` có thể trở nên gần như giống nhau.
...
- thêm các claim không có trong tài liệu.
...
Lệnh trên cài môi trường cho hallucination benchmark - report - demo - PhoBERT detector
